...
START_TEST(test_atan_two_positive) {
  double x = 2.0;
  ck_assert_ldouble_eq_tol(atan(x), s21_atan(x), 1e-6);
}
END_TEST

START_TEST(test_atan_two_negative) {
  double x = -2.0;
  ck_assert_ldouble_eq_tol(atan(x), s21_atan(x), 1e-6);
}
END_TEST
...
START_TEST(test_atan_one_positive) {
  double x = 1;
  ck_assert_ldouble_eq_tol(atan(x), s21_atan(x), 1e-6);
}
END_TEST
...
Suite *suite_atan(void) {
  ...
  tcase_add_test(tc, test_atan_one_positive);
  ...
  tcase_add_test(tc, test_atan_two_positive);
  tcase_add_test(tc, test_atan_two_negative);
