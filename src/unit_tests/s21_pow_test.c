...
START_TEST(test_zero_power) {
  double nums[6] = {1.12,     -34.45,       S21_NAN,
                    -S21_NAN, S21_INFINITY, -S21_INFINITY};
  double p = 0;
  int i = _i;
...
START_TEST(test_one_num) {
  double num = 1.0;
  double p[7] = {1,        1.23,         -1.23,        S21_NAN,
                 -S21_NAN, S21_INFINITY, -S21_INFINITY};
  int i = _i;
...
START_TEST(test_nan_num) {
  double num = S21_NAN;
  double p[8] = {1,       -1,      -11.1, 11.1, S21_INFINITY, -S21_INFINITY,
                 S21_NAN, -S21_NAN};
  int i = _i;
...
START_TEST(test_minus_one_inf_pow) {
  double num = -1;
  ck_assert_ldouble_eq_tol(pow(num, S21_INFINITY), s21_pow(num, S21_INFINITY),
                           1e-6);
}

START_TEST(test_fract_num_neg_inf_pow) {
  double num = 0.123;
  double p = -S21_INFINITY;
  ck_assert_ldouble_infinite(s21_pow(num, p));
  ck_assert_ldouble_infinite(pow(num, p));
}
END_TEST
...
START_TEST(test_neg_inf_pos_num_pow) {
  double num = -S21_INFINITY;
  double p = 12;
  ck_assert_ldouble_infinite(s21_pow(num, p));
  ck_assert_ldouble_infinite(pow(num, p));
...
Suite *suite_pow(void) {
  ...
  tcase_add_test(tc, test_inf_nan_num);
  tcase_add_test(tc, test_neg_inf_pos_num_pow);
